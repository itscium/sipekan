...
//dd($departemens);
        ...
        foreach ($departemens as $index=> $item){
            ...
            $data[$index]['advance'] = $keuangan['travel_advance'];
            $data[$index]['sisa'] = $keuangan['sisa_travel'];
        }
        $wilayah = Wilayah::where('id', Auth::user()->wilayah_id)->first();
        return view('report.departemen.travel-expense.index', compact('departemens', 'wilayah', 'data', 'per_awal', 'per_akhir', 'periode'));
    }
...
//        dd($departemen)
        ...
        return view('report.departemen.travel-expense.detail', compact('per_akhir', 'per_awal', 'periode', 'detail', 'saldo', 'departemen'));
    }
}
