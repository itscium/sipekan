...
class KeuanganController extends Controller
{
    ...
    function get_keuangan ($per_awal, $per_akhir){
        ...
        //get Travel Expense
        $travel_actual = (new A_SALFLDG)->setTable('dbo.ADV_A_SALFLDG')->where('ALLOCATION', '<>', 'C')
            ...
            ->sum('dbo.ADV_A_SALFLDG.AMOUNT');
        $travel_budget = (new A_SALFLDG)->setTable('dbo.ADV_B_SALFLDG')->where('ALLOCATION', '<>', 'C')
            ->where('ACCNT_CODE', $departemen->travel_expense_code)
            ->where('ANAL_T3', $departemen->department_code)
        ...
        $special_travel_budget = (new A_SALFLDG)->setTable('dbo.ADV_B_SALFLDG')->where('ALLOCATION', '<>', 'C')->where('ACCNT_CODE', $departemen->travel_special_code)->where('ANAL_T3', $departemen->department_code)->whereBetween('PERIOD', [$per_awal, date('Y'.'012')])->sum('dbo.ADV_B_SALFLDG.AMOUNT');
        $sisa_special_travel = $special_travel_budget - $special_travel_actual;

        //get Strategic Plan
        $strategic_actual = (new A_SALFLDG)->setTable('dbo.ADV_A_SALFLDG')->where('ALLOCATION', '<>', 'C')->where('ACCNT_CODE', $departemen->strategic_plan_code)->where('ANAL_T3', $departemen->department_code)->whereBetween('PERIOD', [$per_awal, $per_akhir])->sum('dbo.ADV_A_SALFLDG.AMOUNT');
        $strategic_budget = (new A_SALFLDG)->setTable('dbo.ADV_B_SALFLDG')->where('ALLOCATION', '<>', 'C')->where('ACCNT_CODE', $departemen->strategic_plan_code)->where('ANAL_T3', $departemen->department_code)->whereBetween('PERIOD', [$per_awal, date('Y'.'012')])->sum('dbo.ADV_B_SALFLDG.AMOUNT');
        $sisa_strategic = $strategic_budget - $strategic_actual;

        //get Office Expense
        $office_actual = (new A_SALFLDG)->setTable('dbo.ADV_A_SALFLDG')->where('ALLOCATION', '<>', 'C')->where('ACCNT_CODE', $departemen->office_expense_code)->where('ANAL_T3', $departemen->department_code)->whereBetween('PERIOD', [$per_awal, $per_akhir])->sum('dbo.ADV_A_SALFLDG.AMOUNT');
        $office_budget = (new A_SALFLDG)->setTable('dbo.ADV_B_SALFLDG')->where('ALLOCATION', '<>', 'C')->where('ACCNT_CODE', $departemen->office_expense_code)->where('ANAL_T3', $departemen->department_code)->whereBetween('PERIOD', [$per_awal, date('Y'.'012')])->sum('dbo.ADV_B_SALFLDG.AMOUNT');
